...
class BookServiceImplTest {
    ...
    void saveNewBook() {
        ...
        StepVerifier
                .create(publisher)
                .assertNext(book -> assertEquals("Laurentiu", book.getAuthor()))
                .verifyComplete();
    }
    ...
    void delete() {
        Book data = new Book("1", "Laurentiu", "Spring");
        Mono<Book> publisher = bookService
                .createBook(data)
                .flatMap(book -> bookService.deleteBook(book.getId()));
        StepVerifier
                .create(publisher)
                .expectNextMatches(book -> book.getAuthor().equalsIgnoreCase(book.getAuthor()))
                .expectNextCount(0)
                .verifyComplete();
    }
    ...
    void update() throws Exception {
        ...
        StepVerifier
                .create(publisher)
                .expectNextMatches(p -> p.getAuthor().equalsIgnoreCase("Alex"))
                .verifyComplete();
    }
    ...
    void getById() {
        ...
        Mono<Book> publisher = bookService
                .createBook(data)
                .flatMap(book -> bookService.findBookById((book.getId())));
        StepVerifier
                .create(publisher)
                .expectNextMatches(book -> book.toString().equals(data.toString()))
                .verifyComplete();
    }
