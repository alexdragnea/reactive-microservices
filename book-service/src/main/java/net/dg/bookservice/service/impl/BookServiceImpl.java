...
public class BookServiceImpl implements BookService {
	...
	public Mono<Book> deleteBook(String bookId) {
		return bookRepository.findById(bookId).flatMap(book -> bookRepository.deleteById(bookId).thenReturn(book));
	}

	@Override
	public Mono<Book> updateBook(Book book, String bookId) {
		return bookRepository.findById(bookId).flatMap(existingBook -> {
	...
	public Mono<Book> getBookWithRatings(String bookId) {
		Flux<Rating> ratings = webClient.get().uri("http://localhost:8881/rating/book/" + bookId).retrieve()
				.bodyToFlux(Rating.class);
		return ratings.collectList().map(b -> new Book(b)).mergeWith(bookRepository.findById(bookId)).collectList()
